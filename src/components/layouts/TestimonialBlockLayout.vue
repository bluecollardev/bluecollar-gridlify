<template>
  ...
  <div class="content-block testimonial-block-wrapper text-center sm-space-bottom-half" style="position: relative;">
    ...
    <div
      ...
      <div v-if="typeof this.image === 'string' && this.image.length > 3"
           class="circle-composite small space-bottom-half ml-auto mr-auto testimonial-profile-circle xs-hide sm-hide md-hide">
      ...
      <div v-bind:class="classNameString">
        <div class="g-services" style="position: relative;">
          <p class="g-pstyle10 text-center">
            <strong>{{ this.reviewedBy }}</strong><br/>
            <strong><small>{{ this.position }}</small></strong>
          </p>
          <p class="g-pstyle10 text-center">
            <small><em>{{ this.organization }}</em></small>
          </p>
          <div v-if="this.link && this.linkText"
               class="testimonial-link-wrapper"
               style="position: absolute; left: 0; right: 0; z-index: 100; pointer-events: none; text-align: center;">
            <a @click="handleLinkClick"
               style="color: white; font-size: 0.75rem; margin-top: 0.25rem; display: inline-block; cursor: pointer; pointer-events: auto; text-decoration: none;">
              <i class="icon-eye" style="text-decoration: none; font-size: 0.75rem; padding-right: 0.25rem;"></i><span style="text-decoration: underline;">{{ this.linkText }}</span>
            </a>
          </div>
        </div>
      </div>
